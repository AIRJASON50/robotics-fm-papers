...
| 问题 | 具体表现 | 正在探索的方向 |
|------|---------|-----------|
| 精度不够 | VLA 做粗操作好, 亚毫米装配/灵巧操控差 | pi\*0.6 用 offline RL 做 post-training 补精度 |
| 数据太贵 | pi_0 用 10k 小时遥操作 (GPT 用免费互联网数据) | AutoRT 自主采集, DreamZero 想象生成, 仿真合成 |
| 推理太慢 | VLM ~14Hz, 灵巧手需要 50-120Hz | action chunking (低频生成高频执行), 大脑小脑分离 |
| 泛化未知 | 新物体 OK, 新任务/新机器人未知 | 跨 embodiment (Open X), zero-pad action space |
| 安全黑箱 | 不知道模型"为什么"这样动 | 完全未解决 |
...
- Google RT: 定义了 VLA 范式并验证了核心假设, 然后团队出走 (人 > 架构)
- PI: 在 RT-2 基础上解决架构问题 (紧凑 VLM + 连续动作), 用 RL 做 post-training (你的 RL 在这里有用)
