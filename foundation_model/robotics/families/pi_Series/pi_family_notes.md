...
### Phase 6: 长时记忆 + 在线 RL -- MEM + RLT (2026.03)
...
### Phase 7: Compositional Generalization -- pi_0.7 (2026.04)

**要解决的问题**: pi 0.5/0.6 系列即使加了记忆和 RL, 仍然**不会做没见过的任务**, 也无法零样本跨 embodiment。能不能像 LLM 那样, 通过组合训练时学到的 pattern 解决新问题 (compositional generalization)?

**基础模型**: pi_0.6-MEM, backbone Gemma3-4B + 860M action expert, 总参 ~5B

**核心方法: Diverse Prompting (借鉴 LLM 的 prompt expansion)**

```
不是新架构, 是数据工程 + prompt 工程:

把每条 episode 的 prompt 从 "task 描述" 扩展为 5 个 modality:
  1. Task description     ("clean the kitchen")
  2. Subtask instruction  ("open the fridge door")  ← 同 pi_0.5
  3. Subgoal images       未来 ~4 秒的期望状态图    ← 新增
  4. Episode metadata     {speed, quality, mistake} ← 新增
  5. Control mode         {joint, ee}                ← 新增

训练时各 modality 独立 dropout (subgoal 25% 出现, metadata 15% 整体丢)
推理时永远 prompt "quality=5, mistake=false, speed=15th percentile"
对 metadata 做 CFG (β=1.3-2.2) 放大高质量方向
```

**关键洞察 1: metadata 是 advantage conditioning 的简化版**

```
pi*0.6 RECAP:        Value/Advantage → ± token → CFG 时永远输入 "+"
pi_0.7 metadata:    人工标 quality (1-5) → discrete token → CFG 时永远输入 "5"

→ 不需要训 critic, 不需要 reward engineering
→ 直接消化 pi*0.6 RL 训练过程中所有 rollout (包括失败)
→ 等价于"把 specialist 蒸馏回 generalist"
```

**关键洞察 2: 失败/低质量数据反而能用了**

```
传统 BC: 5 分 demo 和 2 分 demo 混训 → 模型 average 两种 mode → 输出垃圾
传统解法: data filtering (费力, 丢信息)

pi_0.7: 把 quality 作为 prompt → 模型学 p(action | obs, quality)
→ 数据越大越好, 即使平均质量下降 (Fig 18 ablation 证明)
→ 之前要扔的失败数据现在变成宝贵的 OOD 状态覆盖
```

**关键洞察 3: 世界模型用作 prompt 生成器 (而不是 policy)**

```
BAGEL 14B (off-the-shelf 图像编辑/生成模型) 微调成 subgoal generator
推理流程: 当前观测 + 子任务文字 → BAGEL 生成 4 秒后期望图 → 喂给 pi_0.7

→ subgoal image 比文字信息量大 (例如"如何握把手"语言说不清)
→ 把 web-scale 视觉先验通过 subgoal 这个接口注入 VLA
→ 在违反数据 bias 的任务 ("Reverse Fridge to Microwave") 上是必需的
→ 异步推理: 世界模型和高层策略在独立线程, VLA 永远用最新 subgoal
```

**结果 (突破性)**:

1. **匹配 RL specialist**: 单一 generalist pi_0.7 直接出箱匹配 pi\*0.6 在 espresso/box-building/laundry 上的成绩, 在 box-building 和 laundry 上**throughput 超过 specialist**
2. **零样本跨 embodiment 匹配人类专家**:
   - 在 UR5e 上折 T 恤 (无 UR5e folding 数据): pi_0.7 task progress 85.6%, 成功率 80%
   - 10 名 top 2% 人类遥操作者首次尝试: 90.9% / 80.6%
   - 模型自动改变策略: 源机器人倾斜抓 → UR5e 用竖直抓 (适应运动学)
3. **新数据采集范式 — Coaching**: 不用遥操作采 action 数据, 人口头一步步指导 → pi_0.7 能做未见任务 (烤红薯/烘 bagel)。Coaching 数据反过来训 high-level 策略 → 完全自主, 无需任何低层 demo
4. **违反数据 bias**: "把垃圾放餐具桶, 餐具放垃圾桶" pi_0.7 能听指令做, 之前的 VLA 全部按数据 prior 行动

**Takeaway 7**: **pi_0.7 的核心信号是"BC + prompt engineering 战胜了 RL"**:
- 不再需要 RECAP 那样训 critic 估 advantage
- 用更轻量的 metadata + diverse data 取得了 RECAP 同等甚至更好效果
- 同时保留 generalist 单模型多任务能力, 而 RL 通常是 specialist
- 失败数据通过 mistake=true 标签变废为宝
- compositional generalization 第一次在机器人上**真正出现了**, 类似 LLM 的 in-context learning

**新发现 vs RL 回归的辨析**:
- **不是 RL 回归** — pi_0.7 反而是"RL 退潮": 用更简单的 conditioned BC 替代了 pi\*0.6 的 RECAP
- **真正的转向**: 数据多样性 + prompt 多样性 + 蒸馏 RL specialist 的 rollout, 而不是显式跑 RL 算法
- 这与 LLM 趋势同构: GPT-4o → o1 (用 CoT 蒸馏) 而不是反复 RLHF
...
| 11 | **Metadata-conditioned BC > 离线 RL (pi_0.7)** | 标 quality/mistake 比训 critic 简单, 效果相当, 还保留 generalist 性质 | 你的 PPO 经验依然有用, 但下一步可以试试用 conditioned BC 替代复杂 RL pipeline |
| 12 | **世界模型可以当 prompt 生成器, 不一定要当 policy (pi_0.7)** | 用 BAGEL 生成 subgoal image 喂给 VLA, 解耦"想象"和"执行" | 不必把 world model 和 policy 端到端绑在一起, 中间用 image 作接口更稳 |
| 13 | **Coaching 范式可能颠覆数据采集 (pi_0.7)** | 用语言一步步指导比遥操作便宜, 还能反过来训 high-level 策略 | 长任务的数据采集可以从 teleoperation 转向 verbal coaching |
...
├── 26_pi07/                        # pi_0.7 Diverse Prompting + 涌现 compositional generalization
