...
### Phase 4: Gemini 时代 (2025-2026) -- "复用 Gemini frontier, 不再训独立 VLM"

**核心问题**: RT 团队走了之后 Google 怎么办? 答案: 不再独立训练 robotics-specific VLM, 而是直接复用 Gemini frontier 的多模态主干 + tool use + reasoning 能力, 加 robot data fine-tune 即成 VLA。

**三个里程碑** (详见 `Gemini_Robotics/Gemini_Robotics_subfamily_notes.md`):

- **Gemini Robotics 1.0** (2025.03, arXiv:2503.20020): 第一代 Gemini-based VLA + ER VLM (基于 Gemini 2.0 Flash)。引入 ERQA benchmark (400 道 embodied reasoning VQA)。能力清单: pointing, trajectory prediction, grasp prediction, multi-view correspondence, 3D bounding box。
- **Gemini Robotics 1.5** (2025.10, arXiv:2510.03342): 三大创新 — (1) **Motion Transfer (MT)** training recipe, 一个 checkpoint 控制 ALOHA + Bi-arm Franka + Apollo humanoid, 零样本跨 embodiment skill transfer; (2) **Embodied Thinking**, 在 action 之前/之间穿插自然语言 reasoning trace, 把 LLM 的 chain-of-thought 完整搬到 robot action 层; (3) **GR-ER 1.5** SOTA embodied reasoning, 作为 agentic orchestrator 调用 GR 1.5 action model。**90%+ 评估在 MuJoCo 仿真里跑**, 仿真-真机 rank consistency 经过校准。
- **Gemini Robotics-ER 1.6** (2026.04.15, blog only): 加 instrument reading 能力 (analog gauges/pressure meters/sight glasses, **93% 准确率, vs ER 1.5 的 23%**), 与 Boston Dynamics 合作发现的 use case。VLA 部分仍是 GR 1.5。

**Phase 4 核心洞察**:

不需要再独立训 robotics VLM — Gemini frontier 已经把所有视觉-语言-推理能力都准备好了, 只缺 action data。Embodied Thinking 是新范式, 把 reasoning 从"高层规划"下沉到"action 之间"。仿真做 90%+ 评估的工程实践重新定义了 robotics evaluation 的成本结构。
...
| 7 | **Gemini Robotics 翻盘: 不再训 robotics-specific VLM, 复用 frontier Gemini + action data** | RT 团队走后 Google 没有再做独立小 VLM, 而是把 Gemini 2.0/3.x frontier 直接当 robotics backbone | Backbone 是否独立训不是必然, 看你能否 piggyback 一个足够强的通用 VLM |
| 8 | **Embodied Thinking 是 robot 层的 chain-of-thought** | GR 1.5 在 action 之前/之间插自然语言 reasoning trace, 不只是高层规划在 think | 长 horizon 任务 thinking 不该只在 orchestrator 层, action model 自己也该 think |
| 9 | **仿真做 90%+ 评估是重新可行的工程范式 (GR 1.5)** | MuJoCo 仿真和真机 rank consistency 校准好之后, ablation 和迭代基本不用真机 | 你已经在用 MuJoCo 做 PPO sim2real, 这个评估闭环可以扩展到 VLA evaluation |
...
+-- Gemini_Robotics/                # Phase 4: Gemini-based robotics (2025-2026)
    +-- Gemini_Robotics_subfamily_notes.md   # 子家族总览
    +-- 25_GR1/                              # Gemini Robotics 1.0 (arXiv:2503.20020)
    +-- 25_GR15/                             # Gemini Robotics 1.5 (arXiv:2510.03342)
                                             # ER 1.6 仅 blog, 无独立 paper
```
