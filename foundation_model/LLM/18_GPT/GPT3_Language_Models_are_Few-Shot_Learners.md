...
- **One-Shot (1S)** is the same as few-shot except that only one demonstration is allowed, in addition to a natural language description of the task, as shown in Figure 2.1. The reason to distinguish one-shot from few-shot and zero-shot (below) is that it most closely matches the way in which some tasks are communicated to humans. For example, when asking humans to generate a dataset on a human worker service (for example Mechanical Turk), it is common to give one demonstration of the task. By contrast it is sometimes difficult to communicate the content or format of a task if no examples are given.
...
Final results are reported on the test set when publicly available, for each model size and learning setting (zero-shot, one-shot, and few-shot). When the test set is private, our model is often too large to fit on the test server, so we report results on the development set. We do submit to the test server on a small number of datasets (SuperGLUE, TriviaQA, PiQa) where we were able to make submission work, and we submit only the 200B few-shot results, and report development set results for everything else.
...
> Alice was friends with Bob. Alice went to visit her friend \_\_\_\_. $\rightarrow$ Bob
...
On Natural Questions (NQs) GPT-3 achieves 14.6% in the zero-shot setting, 23.0% in the one-shot setting, and 29.9% in the few-shot setting, compared to 36.6% for fine-tuned T5 11B+SSM. Similar to WebQS, the large gain from zero-shot to few-shot may suggest a distribution shift, and may also explain the less competitive performance compared to TriviaQA and WebQS.
...
| Setting | En$\rightarrow$Fr | Fr$\rightarrow$En | En$\rightarrow$De | De$\rightarrow$En | En$\rightarrow$Ro | Ro$\rightarrow$En |
|---|---|---|---|---|---|---|
...
Finally, across all language pairs and across all three settings (zero-shot, one-shot, and few-shot), there is a smooth trend of improvement with model capacity. This is shown in Figure 3.4 in the case of few-shot results, and scaling for all three settings is shown in Appendix H.
...
- **4 digit addition (4D+)** -- Same as 3 digit addition, except uniformly from $[0, 10000)$.
- **4 digit subtraction (4D-)** -- Same as 3 digit subtraction, except uniformly from $[0, 10000)$.
- **5 digit addition (5D+)** -- Same as 3 digit addition, except uniformly from $[0, 100000)$.
- **5 digit subtraction (5D-)** -- Same as 3 digit subtraction, except uniformly from $[0, 100000)$.
...
*Table 3.11: Human accuracy in identifying whether short ($\sim$200 word) news articles are model generated. We find that human accuracy (measured by the ratio of correct assignments to non-neutral assignments) ranges from 86% on the control model to 52% on GPT-3 175B.*
...
Mean human accuracy (the ratio of correct assignments to non-neutral assignments per participant) at detecting that the intentionally bad articles were model generated was $\sim 86\%$ where 50% is chance level performance. By contrast, mean human accuracy at detecting articles that were produced by the 175B parameter model was barely above chance at $\sim 52\%$ (see Table 3.11). Human abilities to detect model generated text appear to decrease as model size increases: there appears to be a trend towards chance accuracy with model size, and human detection of GPT-3 is close to chance.
...
*Table 3.12: People's ability to identify whether $\sim$ 500 word articles are model generated (as measured by the ratio of correct assignments to non-neutral assignments) was 88% on the control model and 52% on GPT-3 175B.*
...
We also did a preliminary investigation of how good humans are at detecting longer news articles generated by GPT-3 175B, we selected 12 world news articles from Reuters with an average length of 569 words and generated completions of these articles from GPT-3 with an average length of 498 words (298 words longer than our initial experiments). We found that mean human accuracy at detecting the intentionally bad longer articles from the control model was $\sim 88\%$, while mean human accuracy at detecting the longer articles that were produced by GPT-3 175B was still barely above chance at $\sim 52\%$ (see Table 3.12). This indicates that, for news articles that are around 500 words long, GPT-3 continues to produce articles that humans find difficult to distinguish from human written news articles.
...
*Figure 3.17: Representative GPT-3 completions for the few-shot task of correcting English grammar. Boldface is GPT-3's completions, plain text is human prompts.*
...
- **Reversed Words and Anagrams:** Due to the short length of these tasks, we used 2-grams for filtering (ignoring punctuation). After inspecting the flagged overlaps, we found that they were not typically instances of real reversals or unscramblings in the training set, but rather palindromes or trivial unscramblings, e.g "kayak = kayak". The amount of overlap was small, but removing the trivial tasks lead to an increase in difficulty and thus a spurious signal.

- **PIQA:** The overlap analysis flagged 29% of examples as contaminated, and observed a 3 percentage point absolute decrease (4% relative decrease) in performance on the clean subset. Though the test dataset was released after our training set was created and its labels are hidden, some of the web pages used by the crowdsourced dataset creators are contained in our training set. We found a similar decrease in a 25x smaller model with much less capacity to memorize, leading us to suspect that the shift is likely statistical bias rather than memorization. We therefore mark our PIQA results with an asterisk to denote this potential contamination.
...
A limitation, or at least uncertainty, associated with few-shot learning in GPT-3 is ambiguity about whether few-shot learning actually learns new tasks "from scratch" at inference time, or if it simply recognizes and identifies tasks that it has learned during training. These possibilities exist on a spectrum, ranging from demonstrations in the training set that are drawn from exactly the same distribution as those at test time, to recognizing the same task but in a different format, to learning a skill entirely de novo. Where GPT-3 is on this spectrum may also vary from task to task. Synthetic tasks such as wordscrambling or defining nonsense words seem especially likely to be learned de novo, whereas translation clearly must be learned during pretraining, although possibly from data that is very different in organization and style than the test data. Ultimately, it is not even clear what humans learn from scratch vs from prior demonstrations during pre-training and identifying them at test time would be an advance for language models, but nevertheless understanding precisely how few-shot learning works is an important unexplored direction for future research.
...
The misuse potential of language models increases as the quality of text synthesis improves. The ability of GPT-3 to generate several paragraphs of synthetic content that people find difficult to distinguish from human-written text in Section 3.9.4 represents a concerning milestone in this regard.

#### 6.1.2 Threat Actor Analysis

Threat actors can be organized by skill and resource levels, ranging from low and moderately skilled and resourced actors who may be able to build a malicious product to 'advanced persistent threats' (APTs): highly skilled and well-resourced (e.g. state-sponsored) groups with long-term agendas [SBC+19].
...
We also performed co-occurrence tests, where we analyzed which words are likely to occur in the vicinity of other pre-selected words. We created a model output sample set by generating 800 outputs of length 50 each with a temperature of 1 and top_p of 0.9 for every prompt in our dataset. For gender, we had prompts such as "He was very", "He would be described as", "She was very", "She would be described as". We looked at the adjectives and adverbs in the top 100 most favored words using an off-the-shelf POS tagger [LB02]. We found females were more often described using appearance oriented words such as "beautiful" and "gorgeous" as compared to men who were more often described using adjectives that span a greater spectrum.
...
This appendix contains details on the experiments measuring human ability to distinguish GPT-3-generated synthetic news articles from real news articles. Participants were recruited through Positly, a whitelist of high-performing Mechanical Turk workers. All participants were US-based. Participants were paid $12 for their participation.

*Table E.1: Participant details and article lengths for each experiment to evaluate human detection of $\sim$ 200 word model generated news articles.*
...
The figures in Appendix G illustrate the formatting and phrasing of all the tasks included in the paper. All data comes from the ground truth datasets in this section, and no samples from GPT-3 are included here. (See Figures G.1 through G.51 in the original paper for formatted dataset examples for RACE-h, ANLI R2, RACE-m, PIQA, COPA, ReCoRD, ANLI R1, OpenBookQA, HellaSwag, ANLI R3, ARC (Challenge), SAT Analogies, Winograd, Winogrande, MultiRC, ARC (Easy), StoryCloze, CoQA, Cycled Letters, DROP, LAMBADA, Anagrams 1, Anagrams 2, Natural Questions, QuAC, Symbol Insertion, Reversed Words, SQuADv2, BoolQ, CB, RTE, WiC, WSC, TriviaQA, WebQA, De$\rightarrow$En, En$\rightarrow$De, En$\rightarrow$Fr, Fr$\rightarrow$En, En$\rightarrow$Ro, Ro$\rightarrow$En, Arithmetic 1DC, Arithmetic 2D-, Arithmetic 2D+, Arithmetic 2Dx, Arithmetic 3D-, Arithmetic 3D+, Arithmetic 4D-, Arithmetic 4D+, Arithmetic 5D-, Arithmetic 5D+.)

## Appendix H Results on All Tasks for All Model Sizes

Table H.1 contains scores for every task, setting, and model that we investigate in this paper. The full table is provided in the original paper and contains results across all 8 model sizes (Small, Medium, Large, XL, 2.7B, 6.7B, 13B, 175B) in Zero-Shot, One-Shot, and Few-Shot settings for all benchmarks studied. Key observations from this comprehensive results table include:

- **Cloze and Completion tasks** (HellaSwag, LAMBADA, StoryCloze): Performance scales smoothly with model size across all three settings, with few-shot consistently outperforming zero-shot.
- **QA tasks** (NaturalQS, TriviaQA, WebQs): Dramatic improvements from small to large models, with TriviaQA few-shot reaching 71.2% on the 175B model.
- **Translation tasks**: Strong scaling trends for all language pairs, with translation into English consistently stronger than from English.
- **Reading Comprehension** (CoQA, DROP, QuAC, SQuADv2, RACE): Consistent improvement with scale, though absolute performance varies widely across datasets.
- **SuperGLUE tasks**: Mixed results across tasks, with COPA and ReCoRD showing the strongest performance.
- **NLI tasks** (ANLI R1/R2/R3): Smaller models perform near random chance; only GPT-3 175B shows meaningful improvements above chance.
- **Arithmetic tasks**: Near-zero performance for small models, with a dramatic jump at the 175B scale, particularly for 2-3 digit operations.
- **Word manipulation tasks**: Consistent scaling, though overall accuracy remains modest even for the largest model.
- **SAT Analogies**: The 175B model achieves 65.2% few-shot, above the average college applicant score of 57%.

Figures H.1 through H.11 in the original paper show scaling curves for all SuperGLUE tasks, SAT and Winograd tasks, arithmetic tasks, cloze and completion tasks, common sense reasoning tasks, QA tasks, reading comprehension tasks, ANLI rounds, word scramble tasks, and translation tasks respectively.
...
[AJF19] Roee Aharoni, Melvin Johnson, and Orhan Firat. Massively multilingual neural machine translation. In *Proceedings of the 2019 Conference of the North American Chapter of the Association for Computational Linguistics*, 2019.
...
[DMST19] Marie-Catherine De Marneffe, Mandy Simons, and Judith Tonhauser. The CommitmentBank: Investigating projection in naturally occurring discourse. 2019.
...
[IBGC+14] Mohit Iyyer, Jordan Boyd-Graber, Leonardo Claudino, Richard Socher, and Hal Daume. A neural network for factoid question answering over paragraphs. In *Empirical Methods in Natural Language Processing*, 2014.
...
[KCR+18] Daniel Khashabi, Snigdha Chaturvedi, Michael Roth, Shyam Upadhyay, and Dan Roth. Looking beyond the surface: A challenge set for reading comprehension over multiple sentences. In *NAACL*, 2018.
...
[LGH+15] Xiaodong Liu, Jianfeng Gao, Xiaodong He, Li Deng, Kevin Duh, and Ye-Yi Wang. Representation learning using multi-task learning for semantic classification and information retrieval. In *Proceedings of the 2015 Conference of the North American Chapter of the Association for Computational Linguistics*, 2015.
...
[TL05] Peter D. Turney and Michael L. Littman. Corpus-based learning of analogies and semantic relations. *CoRR*, cs/cs/0508103, 2005.
