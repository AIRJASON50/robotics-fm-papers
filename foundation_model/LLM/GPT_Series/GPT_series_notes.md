...
注意: 这 10 篇工作有明确的层级区分 -- 有些是范式突破 (改变了整个领域的做法), 有些是范式验证 (证明已有范式在新领域有效)。

**范式突破 (改变了"怎么做 AI")**:

| 论文 | 层级 | 核心突破 | 提出的解法 |
|------|------|---------|---------|
| GPT-1 (2018) | 范式定义 | 确立 pre-train + fine-tune | Generative pre-training + discriminative fine-tuning |
| GPT-2 (2019) | 范式推进 | 去掉 fine-tune, zero-shot | 更大模型 + 更多数据 = unsupervised multitask learning |
| Scaling Laws (2020.01) | 理论基础 | 量化了"大力出奇迹"的规律 | 发现 power-law, 为 GPT-3 提供 scaling 理论指导 |
| GPT-3 (2020.05) | 范式突破 | In-context learning 涌现 | 175B 参数, few-shot 无需梯度更新 |
| RLHF-Summarize (2020.09) | 方法奠基 | 首次证明 RLHF 在大 LM 上有效 | Human preference -> reward model -> PPO |
| InstructGPT (2022.03) | 范式集大成 | Alignment > Scale (1.3B > 175B) | SFT + RM + PPO 三步法完整确立 |
| GPT-4 (2023.03) | 能力扩展 | 多模态 + predictable scaling | Multimodal + RLHF + 小模型预测大模型性能 |
...
> - **PPO** = Proximal Policy Optimization, 近端策略优化 -- 一种强化学习算法, 通过限制每步更新幅度来保证训练稳定性
> - **SFT** = Supervised Fine-Tuning, 监督微调 -- 用人工编写的高质量回答直接微调模型, 是 RLHF pipeline 的第一步
> - **RM** = Reward Model, 奖励模型 -- 输入 (prompt, response), 输出一个标量分数表示回答好坏, 从人类比较数据中训练得到
> - **Alignment > Scale**: 经过 RLHF 对齐的 1.3B 小模型, 在人类偏好评测中胜过未对齐的 175B GPT-3. "让模型听话"比"让模型更大"更重要

**范式验证 (证明"同一个 base model + 领域 fine-tune = 新能力")**:

| 论文 | 验证了什么 | Base Model | Fine-tune 方式 |
|------|---------|-----------|---------------|
| Codex (2021.07) | 领域 fine-tune 激活专业能力 | GPT-3 | Supervised on 159GB 代码 |
| WebGPT (2021.12) | LLM 可以学会使用外部工具 | GPT-3 | BC + RLHF on 浏览器交互 |
| ChatGPT (2022.11) | InstructGPT 方法可产品化 | GPT-3.5 | SFT + RLHF on 对话数据 |

> - **BC** = Behavior Cloning, 行为克隆 -- 用监督学习直接模仿人类操作轨迹, 是最简单的 imitation learning 方法

Codex/WebGPT/ChatGPT 不是和 GPT-1/2/3 同层次的范式突破, 而是 **验证了 GPT-3 作为通用 base model 的威力** -- 同一个预训练模型, 换不同数据 fine-tune 就能获得代码生成、工具使用、对话等完全不同的能力。这个 "一个 base model, 多种 fine-tune" 的模式后来被机器人领域直接继承 (PaliGemma -> pi_0)。

### 完整演进脉络

```
=== Phase 1: Pre-training 范式确立 (2018-2019) ===

GPT-1 (2018.06): "pre-train + fine-tune" 范式
  |  (insight: 去掉 fine-tuning 也能工作)
  v
GPT-2 (2019.02): "pre-train only" -- zero-shot task transfer
  |  (insight: 放大模型和数据, zero-shot 持续提升)
  |  (问题: 该怎么放大? 放大多少?)

=== Phase 2: Scale is all you need (2020) ===

Scaling Laws (2020.01): 发现 power-law, 为 GPT-3 提供理论指导
  |  (结论: 优先扩大模型, 不必训练到收敛, 性能可预测)
  v
GPT-3 (2020.05): 175B 参数, in-context learning 涌现
  |  (成就: few-shot 接近 fine-tuned SOTA, 无需梯度更新)
  |  (问题: 模型能力很强, 但不听话、不可靠、有毒性)

=== Phase 3: Alignment -- 从 "能做" 到 "好用" (2020-2022) ===

RLHF-Summarize (2020.09): 首次在大规模 LM 上验证 RLHF 有效
  |  (证明: human feedback > supervised learning, 即使模型更小)
  |
  +---> Codex (2021.07): 领域 fine-tune (代码)
  |       (证明: pre-trained LM + domain data = 专业能力涌现)
  |       (催生产品: GitHub Copilot, 2021.10)
  |
  +---> WebGPT (2021.12): RLHF + 工具使用 (浏览器)
  |       (证明: 模型可以学会用外部工具, human feedback 提升 factuality)
  v
InstructGPT (2022.03): 集大成 -- SFT -> Reward Model -> PPO
  |  (核心发现: 1.3B InstructGPT 人类偏好率 > 175B GPT-3)
  |  (意义: alignment 比 scale 更重要)
  v
ChatGPT (2022.11.30): InstructGPT 方法 + 对话优化 -> 产品爆发
  |  (2个月达到 1 亿用户, 引爆 AI 行业)

=== Phase 4: Multimodal + Safety (2023+) ===

GPT-4 (2023.03): multimodal + RLHF + predictable scaling
  |  (不再公开技术细节, 转向竞争性产品)
  v
GPT-4o (2024.05): omni-modal (text + image + audio, native)
  v
o1 (2024.09): chain-of-thought reasoning at inference time
```
