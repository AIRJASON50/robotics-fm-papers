...
```
1. 速度慢: 每步只出一个 token, 不能并行
2. 精度丢失: 连续值 (如关节角) 必须离散化为 token (256 bins ≈ 0.7° 精度)
3. 不支持 action chunking: 一次只能出一个动作 token, 不能一次出 50 步

RT-2 的困难:
  50Hz 灵巧操控需要每秒 50 x 7 DoF = 350 个 token
  autoregressive 逐个生成, 速度跟不上
  且 256 bins 离散化精度不够
```

**这正是 pi_0 放弃 autoregressive、改用 flow matching 的原因**: flow matching 直接输出连续值 (无离散化), 支持 action chunking (一次出 50 步), 10 步积分比逐 token 快。
...
| Q2 | 以为 few-shot 是微调 | LLM few-shot 不更新权重; 机器人 few-shot 才更新权重 |
| Q3 | "更多就是更好" | 不是更多更好, 是模型和数据要匹配 |
| Q5 | 以为限制是 compounding error | 是速度慢 + 离散化精度丢失 + 不支持 action chunking |
| Q6 | 不了解 MoE | pi_0 的 dual-expert 就是最简形式的 MoE |
| Q8 | 不知道 | embedding 是静态查表, attention 后是上下文化的动态表征 |
