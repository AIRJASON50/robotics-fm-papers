...
ViT 要回答一个根本性问题: **纯 Transformer 架构 (不含任何卷积) 能否在视觉任务上达到甚至超越 CNN?**

2020 年的背景:
- NLP 领域 Transformer 已成为标准架构 (BERT, GPT), 模型规模突破 100B 参数且性能未饱和
- CV 领域 CNN 仍占统治地位 (ResNet, EfficientNet), 虽有多种尝试将 self-attention 引入视觉 (local attention, sparse attention, axial attention), 但无一能替代 CNN
...
Linear Projection: 每个 patch flatten 后通过可训练线性层映射到 D 维
  ...
  v
Prepend [CLS] token: 添加一个可学习的 class token
  |
  v
Add Position Embeddings: 加上可学习的 1D position embedding (shape: (N+1) x D)
  |
  v
Transformer Encoder: L 层标准 encoder (Pre-LN 变体)
  |
  v
...
z_l  = MLP(LN(z'_l)) + z'_l           (feed-forward + residual)
```
...
**Fine-tuning**: SGD with momentum 0.9, batch size 512, 分辨率通常提升到 384 (甚至 512/518), position embedding 做 2D 双线性插值适应新分辨率
...
**直觉解释**: CNN 通过 locality 和 translation equivariance 这两个 inductive bias 来处理图像。ViT 的 patch embedding 只保留了最弱的 locality 假设 (patch 内部的像素相关), 完全放弃了平移不变性和层级结构。代价是需要更多数据来学习这些关系; 收益是模型容量不再被 CNN 的固有结构限制, 可以学到更灵活的表征。

**为什么对 robotics 很重要**: 后续所有 VLA 模型 (CLIP, RT-2, pi_0, GR00T N1) 的 vision encoder 都基于 ViT 的 patch embedding。这个设计有几个对机器人至关重要的属性:
- **输出是一组 spatial tokens** (不是单个向量): 每个 patch token 保留了空间位置信息, 下游模型 (如 GR00T N1 的 DiT) 可以通过 cross-attention 选择性地关注图像的不同区域
...
R50+ViT (用 ResNet 做 stem, 输出 feature map 作为 ViT 的输入):
...
7. **只输出全局特征或 CLS token**: 原版 ViT 的分类 head 只取 CLS token, 丢弃了所有 spatial information。Robotics 需要 spatial features (物体位置、抓取点), 后续工作通过取消 CLS token 或使用 `unpooled` 输出解决。代码中已有 `classifier='unpooled'` 和 `classifier='token_unpooled'` 选项。
...
| 视觉-语言对齐 | CLIP (2021) | Image encoder | Contrastive pre-training, 获得 language-aligned visual features |
| 视觉-语言理解 | PaliGemma / Eagle-2 | VLM 的 vision tower | 与 LLM decoder 联合训练, 获得 grounded visual reasoning |
| 机器人控制 | pi_0 / GR00T N1 | VLA 的 vision encoder | 视觉特征驱动动作生成 |
...
| **核心创新** | Patch tokenization | Contrastive alignment | Iterative denoising |
| **对 VLA 的贡献** | 视觉 token 化范式 | Language-aligned vision backbone | Action generation 框架 |
| **训练数据** | 标注分类数据 (ImageNet-21k/JFT) | 互联网 image-text pairs (400M) | 无标注图像 |
| **输出** | 单个 class label | Image/text embedding pair | 生成的图像 |
| **Scaling 特性** | 数据量决定 ViT vs CNN 优劣 | Compute-performance log-log linear | FID 随模型增大单调下降 |
