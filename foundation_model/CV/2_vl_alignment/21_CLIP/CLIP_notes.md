...
双塔结构 (dual-encoder, 两个独立的编码器, 不共享参数):
- **Image Encoder**: ResNet (ModifiedResNet) 或 ViT (Vision Transformer)
- **Text Encoder**: Transformer (causal masked self-attention, 12 layers, 512 width, 8 heads, 63M params)
- **Projection**: 两个 encoder 的输出各自通过线性投影 (W_i, W_t) 映射到共享的 multi-modal embedding space (多模态嵌入空间)

注意: CLIP 是 encoder-only 模型, 只能做匹配 (图文配对/检索), 不能生成文字或动作。能生成的是后续接了 LLM decoder 的 VLM (Vision-Language Model, 如 PaliGemma = SigLIP encoder + Gemma decoder)。
...
这个 symmetric cross-entropy loss (对称交叉熵损失) 本质是 multi-class N-pair loss, 也叫 InfoNCE (Info Noise-Contrastive Estimation) loss。直觉: 对角线 (配对的) 概率应该高, 其他 (不配对的) 应该低, 和分类 loss 一样, 只是 "类别" 变成了 "batch 里的哪个文字"。
...
直觉: text encoder 是一个 hypernetwork (超网络), 根据文本动态生成 classifier weights。和 ViT 的区别: ViT 分类的"库"是固定的 1000 个类别 (训练时定死), CLIP 的"库"是你给的任意文字 (推理时随便换), 这就是"开放词表"。
...
CLIP 的核心贡献是证明了: 用 contrastive learning (对比学习) 对齐 image-text 表征, 可以学到一个 zero-shot 可迁移、distribution shift robust 的 vision backbone。这个 insight 直接催生了 VLA 模型的技术路线 -- 用大规模 web data 预训练 vision-language model, 再 fine-tune 到 robot control。理解 CLIP 是理解 RT-2, pi_0, GR00T N1 等现代 VLA 模型的基础。

### CLIP 的局限 (对机器人)

CLIP 输出的是 [CLS] token 压缩成的**全局向量**, 丢失了空间信息:
- 知道"图里有红色杯子", 不知道"杯子在左边还是右边"
- 对分类/检索够用, 对机器人定位不够
- 后续 SigLIP / PaliGemma 改成保留全部 spatial tokens (196 个 patch token), 解决了这个问题

### CLIP 的遗产

| 层面 | 被继承的 | 被替代的 |
|------|---------|---------|
| 方法 | 对比学习对齐图文 → SigLIP 改进了 loss 但思想一致 | CLIP 的 softmax loss → SigLIP 改成 sigmoid (不需要超大 batch) |
| 架构 | ViT 做图像 + Transformer 做文字 → 所有 VLM 的基本结构 | CLIP 的 text encoder → 被 LLM (Gemma, Llama) 替代 |
| Backbone | CLIP ViT 直接被 Diffusion Policy 使用 | 被 SigLIP ViT (pi_0) 和 DINOv2 (OpenVLA) 替代 |

pi_0 用的不是 CLIP, 而是 CLIP 的"后代" SigLIP — 改进了 loss function, 但核心思想 (对比学习对齐图文) 完全继承自 CLIP。
